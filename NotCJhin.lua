...
function NotC:MenuRTarget(v,t)
    if NotC.Combo.R.BlackList[v.charName] ~= nil then
    else
        NotC.Combo.R.BlackList:MenuElement({id = v.charName, name = "Blacklist: "..v.charName, value = false})
    end
end
...
function NotC:useQonTarget(target)
...
function NotC:useWkill(target,wPred)
...
function NotC:useEbrainAFK(target,ePred)
...
function NotC:GetRTarget(closeRange,maxRange)
    local tick = GetTickCount()
